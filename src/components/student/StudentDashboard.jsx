...
import { BookOpen, Award, BookMarked, Bell, FileText, User, Book, BarChart2, Calendar, Clock, ChevronRight, Search, Video, FolderOpen, Upload } from 'lucide-react';
import { useAppContext } from '../../context/AppContext';
import ResourcesSection from './ResourcesSection';

const StudentDashboard = ({ user, onStartExam, onNavigateToGat, onNavigateToGatTraining, onNavigateToGatCourses, onLogout }) => {
  ...
  const [activeTab, setActiveTab] = useState('exams'); // 'exams' or 'resources'
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className={`${darkMode ? 'bg-gradient-to-r from-blue-900 to-purple-900' : 'bg-gradient-to-r from-blue-500 to-purple-600'} rounded-lg shadow-lg p-6 mb-6 text-white`}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            ...
            <div className="mt-4 md:mt-0 flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2">
              <button
              ...
              <button
                onClick={onNavigateToGatCourses}
                className="px-4 py-2 bg-indigo-800 text-white rounded-md font-medium hover:bg-indigo-700 transition-colors duration-200 flex items-center"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div 
            ...
            className={`${darkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-50'} rounded-lg shadow-md p-6 cursor-pointer transition-colors duration-200 flex`}
          >
            <div className={`${darkMode ? 'bg-indigo-900/50' : 'bg-indigo-100'} p-4 rounded-lg mr-4`}>
              <Video className={`h-8 w-8 ${darkMode ? 'text-indigo-400' : 'text-indigo-600'}`} />
            </div>
            <div>
              <h2 className="text-xl font-bold mb-2">GAT Courses</h2>
              <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-4`}>
                Access comprehensive video lessons and study materials for the GAT exam.
              </p>
              <button 
                className={`inline-flex items-center px-4 py-2 rounded-md ${
                  darkMode
        ...
        {/* Tab navigation for Exams and Resources */}
        <div className="mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {/* Main content area - Available exams or Resources */}
          <div className="md:col-span-2">
            {activeTab === 'exams' ? (
              // Exams content
              <>
